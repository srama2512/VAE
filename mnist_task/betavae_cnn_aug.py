...
class vae(object):
    ...
    def _create_network_(self):
        ...
        self.X_placeholder = tf.placeholder(tf.float32,[None, 40, 40, 1])

        self.fc_size = 32*4*4 # Size to flatten to after 2nd convolution 
    ...
    def getEncoder(self):
        ...
        # Convolution layer 2
        self.conv_w_2 = getWeight([3, 3, 64, 64])
        self.conv_b_2 = getBias([64])
        conv_2 = tf.nn.relu(tf.nn.conv2d(conv_1, self.conv_w_2, \
                     strides=[1, 2, 2, 1], padding='VALID') + self.conv_b_2)

        # Convolution layer 3
        self.conv_w_3 = getWeight([3, 3, 64, 32])
        self.conv_b_3 = getBias([32])
        conv_3 = tf.nn.relu(tf.nn.conv2d(conv_2, self.conv_w_3, \
                     strides=[1, 2, 2, 1], padding='VALID')+self.conv_b_3)
        
        # Projection layer
        conv_3_reshaped = tf.reshape(conv_3, [-1, self.fc_size])

        # Get mu(X) and log_Sigma_diag(X)
        self.mu_weight = getWeight([self.fc_size, self.z_size])
        self.mu_bias = getBias([self.z_size])
        self.mu_X = tf.matmul(conv_3_reshaped, self.mu_weight) + self.mu_bias

        self.Sigma_weight = getWeight([self.fc_size, self.z_size])
        self.Sigma_bias = getBias([self.z_size])
        self.log_Sigma_X_diag = tf.matmul(conv_3_reshaped, self.Sigma_weight) + self.Sigma_bias
    ...
    def getGenerator(self):
        ...
        proj_reshaped = tf.nn.relu(tf.reshape(gen_proj, [self.batch_size, 4, 4, 32]))
        ...
        trans_conv_1 = tf.nn.relu(tf.nn.conv2d_transpose(proj_reshaped, self.trans_conv_w_1, output_shape=[self.batch_size, 9, 9, 64], strides=[1, 2, 2, 1], padding='VALID') + self.trans_conv_b_1)

        # Transposed Convolution layer 2
        # Note: conv2d_transpose expects the filter size to be specified
        # as (height, width, OUT_channels, IN_channels)
        self.trans_conv_w_2 = getWeight([3, 3, 64, 64])
        self.trans_conv_b_2 = getBias([64])
        trans_conv_2 = tf.nn.conv2d_transpose(trans_conv_1, self.trans_conv_w_2, output_shape=[self.batch_size, 19, 19, 64], strides=[1, 2, 2, 1], padding='VALID')+self.trans_conv_b_2

        # Transposed Convolution layer 3
        # Note: conv2d_transpose expects the filter size to be specified
        # as (height, width, OUT_channels, IN_channels)
        self.trans_conv_w_3 = getWeight([4, 4, 1, 64])
        self.trans_conv_b_3 = getBias([1])
        trans_conv_3 = tf.nn.conv2d_transpose(trans_conv_2, self.trans_conv_w_3, output_shape=[self.batch_size, 40, 40, 1], strides=[1, 2, 2, 1], padding='VALID')+self.trans_conv_b_3

        # Generated output
        self.output = tf.nn.sigmoid(trans_conv_3)
