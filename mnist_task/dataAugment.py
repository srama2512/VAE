...
augmented_inputs = np.zeros((reshaped_input.shape[0]*num_trans, CANVAS_DIM, CANVAS_DIM, 1))

aug_counter = 0
for i in  range(batch_size):
    canvas_image = np.zeros((CANVAS_DIM, CANVAS_DIM))
    canvas_image[(y_center-h_half):(y_center+h_half), (x_center-w_half):(x_center+w_half)] = reshaped_input[i,:,:,0].copy()
    ...
    if params['rotation'] == 1:
        ...
        for r in range(rotations.shape[0]):
            ...
            augmented_inputs[aug_counter,:,:,0] = image_rotated.copy()
            aug_counter += 1
            if params['debug_mode'] == 1:
                plt.subplot(121)
    ...
    if params['translation_x'] == 1:
        ...
        for t in range(translations.shape[0]):
            ...
            augmented_inputs[aug_counter,:,:,0] = image_x_trans.copy()
            aug_counter += 1
            if params['debug_mode'] == 1:
                plt.subplot(121)
                plt.imshow(canvas_image)
    ...
    if params['translation_y'] == 1: 
        ...
        for t in range(translations.shape[0]):
            ...
            augmented_inputs[aug_counter,:,:,0] = image_y_trans.copy()
            aug_counter += 1
            if params['debug_mode'] == 1:
                plt.subplot(121)
                plt.imshow(canvas_image)
    ...
    if params['scaling'] == 1: 
        ...
        for s in range(scaling.shape[0]):
            ...
            scaled_input = transform.rescale(reshaped_input[i,:,:,0], scaling[s])
            ...
            augmented_inputs[aug_counter,:,:,0] = canvas_image_scaled.copy()
            aug_counter += 1
            if params['debug_mode'] == 1:
                plt.subplot(121)
                plt.imshow(canvas_image)
